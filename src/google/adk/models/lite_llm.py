...
def _schema_to_dict(schema: types.Schema) -> dict:
    """Recursively converts a types.Schema to a JSON-serializable dictionary with all type fields as strings."""
    schema_dict = schema.model_dump(exclude_none=True)

    # Convert 'type' field to string if it's an enum
    if "type" in schema_dict:
        t = schema_dict["type"]
        if hasattr(t, "value"):
            schema_dict["type"] = TYPE_LABELS.get(t.value, t.value.lower())
    ...
    if "items" in schema_dict:
        ...
        if isinstance(items, types.Schema):
            ...
            # Sometimes items is a dict, not a Schema
            schema_dict["items"] = _schema_to_dict(types.Schema.model_validate(items))
        elif isinstance(items, list):
            schema_dict["items"] = [
                _schema_to_dict(types.Schema.model_validate(i)) if isinstance(i, dict) else _schema_to_dict(i)
                for i in items
            ]
    ...
    if "properties" in schema_dict:
        ...
        for key, value in schema_dict["properties"].items():
            if isinstance(value, types.Schema):
                ...
                properties[key] = _schema_to_dict(types.Schema.model_validate(value))
            else:
            ...
            if "type" in properties[key]:
                ...
                if hasattr(t, "value"):
                    properties[key]["type"] = TYPE_LABELS.get(t.value, t.value.lower())
                elif isinstance(t, str):
                    properties[key]["type"] = t.lower()
